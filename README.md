...
```text
Arduino → arduinowithnotifs.py → FastAPI (main_complete.py) → cooling_system.db
                                               ↓
                                     frontend_dashboard.html (via servesite.py)
                                               ↓
                                   Gemini AI (only when TDS > 1000 ppm)
                                               ↓
                                Email/SMS alerts + dashboard visualization
